...
# search space is gamma = 2^(-1:1)/50
# cost = 2^(-1:1)
# by default gamma parameter needed for all kernels except linear (default: 1/(data dimension))
# cost	cost of constraints violation (default: 1)—it is the ‘C’-constant of the regularization term in the Lagrange formulation.
cv.svm.results <- tune.svm(train.exp.mat,train.labels,gamma = 2^(-1:1)/50, cost = 2^(-1:1),probability = TRUE)
