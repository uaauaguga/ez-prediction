...
```{R}
## Suppose we some how reduce the search tp 8 features, we want to 4 most important ones
## features.started is 8 features we start from
features.started <- tail(rownames(tumor.vs.normal.de$table),8)
all.combinations <- combn(1:8, 4, simplify = FALSE)
...
cv.for.FS <- function(x,y){
    ...
    rf.cv <- train(x,y,
                 ...
                 ntree = 100,
                 tuneLength = 1,
                 trControl = trainControl(number = 5,method = "repeatedcv",repeats = 1,savePredictions = T,classProbs = T))
...
# A parallel version, should be quicker
# library(parallel)
# cv.aurocs1 <- mclapply(all.combinations,function(indices,x,y){cv.for.FS(x[,indices],y)},x=exp.mat.started,y=labels,mc.cores=4) 

features.selected <- features.started[all.combinations[[which.max(unlist(cv.aurocs))]]]
